...
add_route() {
    ...
    for i in "${route[@]}"; do
        ip route add ${i} dev ${TUNNEL_NAME} metric ${TUNNEL_MARK} src 11.11.0.10
        echo "Executed: ip route add ${i} dev ${TUNNEL_NAME} metric ${TUNNEL_MARK}" >> /tmp/vpn_debug.log
    done
    echo "Executing IPTABLES FORWARD rule: /sbin/iptables -t mangle -A FORWARD -o ${TUNNEL_NAME} -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu" >> /tmp/vpn_debug.log
    /sbin/iptables -t mangle -A FORWARD -o ${TUNNEL_NAME} -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu
    echo "Executed IPTABLES FORWARD rule" >> /tmp/vpn_debug.log
    echo "Executing IPTABLES INPUT rule: /sbin/iptables -t mangle -A INPUT -p esp -s ${TUNNEL_REMOTE_ENDPOINT} -d ${TUNNEL_LOCAL_ENDPOINT} -j MARK --set-xmark ${TUNNEL_MARK}" >> /tmp/vpn_debug.log
    /sbin/iptables -t mangle -A INPUT -p esp -s ${TUNNEL_REMOTE_ENDPOINT} -d ${TUNNEL_LOCAL_ENDPOINT} -j MARK --set-xmark ${TUNNEL_MARK}
    echo "Executed IPTABLES INPUT rule" >> /tmp/vpn_debug.log
